"R Script for the visualisation of regulated proteins compared to baseline separately in PRO and CTR in plasma.
Output: 3D Volcanos"
...
# === Load dataset ===
file <- "/Users/majaewen/Documents/Uni/6_Semester/Daten/Volcanos/Np1_Ttest_in_controls.txt" #Enter Np1_Ttest_in_profis.txt to generate Volcanos for PRO
...
prot_toplist_long <- merge(p_value_long, adj_p_value_long, 
                           by = c("protein", "Gene name", "Comparison"))
prot_toplist_long <- merge(prot_toplist_long, estimate_long, 
                           by = c("protein", "Gene name", "Comparison"))

# === Rename for consistency ===
setnames(prot_toplist_long, c("protein", "Gene name"), c("ID", "Gene"))
  ...
  layout(
    ...
    scene = list(
      ...
      yaxis = list(
        ...
        range = c(max(prot_toplist_filtered$logFC), min(prot_toplist_filtered$logFC)),     
        autorange = FALSE,
        tickfont = list(size = 14)
      ),
      zaxis = list(
...
htmlwidgets::saveWidget(as_widget(phos3D_J),
                        "/Users/majaewen/Documents/Uni/6_Semester/Daten/Volcanos/Plasma_CTR.html")